	...
	.factory('Fabric', [
		...
		function (FabricWindow, $timeout, $window, FabricCanvas, FabricDirtyStatus) {
			...
			return function (options) {
				...
				// Undo nad Redo
				// ============================================================
				self.updateState = function save() {
					self.redo = [];
					self.redoStatus = true;
					if (self.state) {
						self.undoStatus = false;
						self.undo.push(self.state);
					}
					self.state = self.getJSON();
				}

				self.canvasUndo = function () {
					self.redo.push(self.state);
					self.state = self.undo.pop();
					self.undoStatus = true;
					self.redoStatus = true;
					canvas.clear();
					canvas.loadFromJSON(self.state, function () {
						canvas.renderAll();
						self.redoStatus = false;
						if (self.undo.length) {
							self.undoStatus = false;
						}
					});
